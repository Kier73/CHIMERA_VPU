...
class FeedbackLoop {
public:
    FeedbackLoop(std::shared_ptr<HardwareProfile> hw_profile, double quark_threshold = 0.15, double learning_rate = 0.1);

    void learn_from_feedback(const LearningContext& context, double predicted_flux, const ActualPerformanceRecord& record);

private:
    void apply_learning(double& belief, double observed, double predicted);
